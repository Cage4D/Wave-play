...
export function LikedSongs() {
  ...
  // commit: feat: filter full track list down to only liked tracks using the Set
  const likedTracks = tracks.filter((track) => likedSongs.has(track.id));

  return (
    // commit: feat: responsive padding — tighter on mobile, fuller on desktop
    <div className="p-4 md:p-6 lg:p-8">
      <div className="mb-6 md:mb-8">
        {/* commit: feat: header row — heart badge and title scale down on mobile */}
        <div className="flex items-center gap-3 md:gap-4 mb-4">
          ...
          <div>
            ...
            {/* commit: feat: show live count of liked tracks under the title */}
            <p className="text-sm md:text-base text-zinc-400">{likedTracks.length} songs</p>
          </div>
        </div>
      </div>

      {/* commit: feat: conditional render — empty state or track grid depending on liked count */}
      {likedTracks.length === 0 ? (
        // commit: feat: empty state — centred heart icon and prompt to like tracks, tighter padding on mobile
